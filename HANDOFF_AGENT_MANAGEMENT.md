...
| Connectors | 真实图标、分类筛选 chip、OAuth WebView、成功反馈 | ✅ 核心数据/交互已迁移并回归 | ✅ 真实远程 logo、分类筛选、OAuth URL 打开、前台 silent refresh、OAuth 返回后新增 connected toolkit 成功提示、管理页 connect/disconnect 已有实现与 presenter 测试；2026-06-17 已重跑 list/manage 测试通过。剩余：iOS 撒花动效/最终视觉 polish 与设备截图。 |
...
| Credits | Balance/Daily Usage/Usage Ranking 三 tab、sparkline、Top Up、交易记录 | ✅ 数据层、Settings 入口、Topup PaymentSheet 已迁移 | 🟡 Credits period 已对齐 iOS `sevendays/thirtydays/all`；Topup 已有 state/presenter/子页面、PaymentIntent 创建、官方 Stripe `PaymentSheet` bridge、PaymentSheet result 回写、status polling；Settings root 现在通过 `PreferencesFlowNode` refresh flow 在 topup 完成后触发 `PreferencesRootPresenter.loadCreditBalance()`，对齐 iOS `loadCreditBalance()`。2026-06-17 已刷新 `CreditsPresenterTest` / `CreditsViewTest` / `CreditFormattersTest` / `TopupPresenterTest`，全部通过。剩余：视觉/图标/按钮样式和真机截图 polish |
...
| Voice Library | My/Public tab、录音（mic/录/放）、列表试听、删除确认、分享/导入 | ✅ 功能链路已迁移并回归 | ✅ Mine/Public、catalog、save/delete/share/import/delete notice、presenter-owned preview state、下载缓存式 preview、recording upload API、current-recording state/events、`RECORD_AUDIO` 权限、原生 m4a/base64 录音 bridge + 已录音本地回放/进度/seek/scrub 状态与设计系统 waveform UI 已接入；已录音 m4a 会解码成真实 waveform samples，不再使用固定 demo 波形。2026-06-17 已重跑 `VoiceLibraryPresenterTest`、`VoicePreviewFileCacheTest`、`VoiceRecordingWaveformNormalizerTest` 通过。剩余：视觉和真机录音/拖动手势对齐 |
| Vault 管理 | 独立 `VaultManagementScreen` + `VaultEditScreen`（key/value/desc 增删改查） | ✅ 功能链路已迁移并回归 | ✅ 删除接口已对齐 iOS key 路由；列表/search/delete 与编辑 value 加载/create/update 校验已有 presenter 测试，2026-06-17 已重跑 `VaultManagementPresenterTest` 与 `VaultEditPresenterTest` 通过。剩余：本地化和视觉 polish |
...
1. **JsonRender 组件目录补全**（task #14 follow-up）：`data-ui-spec`/`data-json-render`/`data-spec` 已接第一版 `JsonSpecRender`；文本 `$state`、节点 `visible`、基础 `repeat` 局部 item state、markdown `json/spec` code-block 分流已支持。
   ...
   - 2026-06-17：JsonRender 解析层对齐 iOS `docs/json-render.md` 的 payload wrapper，支持 `{type:"flat", spec}`、`{type:"nested", spec}`、`{type:"patch", patch}`；nested spec 会 flatten 成 Android render model；元素有 `props` 时也会保留 iOS 顶层 `visible/on/repeat/template` 等字段，避免状态语义丢失。验证：`:features:messages:impl:testDebugUnitTest --tests JsonSpecRenderTest` + `:features:messages:impl:compileDebugKotlin` 通过。剩余：真实复杂非 URL 事件触发需要上层 action dispatcher/host 协议。
...
3. **Selected/active 视觉状态统一**：当前 Android 多处 selected state 不是 iOS 的圆角 capsule/pill，而是局部 `background(color)` 或缺少同层 shape，导致 tab/chip/选中块出现直角。需要先抽统一的 rounded selectable/token，再替换 `ToolCallRootCard` tabs、Finance 二级 tabs、Weather forecast pills、Composer skill picker、room topbar/menu chips、tool card 内所有 selected/active/pressed 状态；Compose 里不允许继续散落无 shape 的 selected background。验收：深浅色、所有 tool cards、skill picker、room menu 展开态截图里选中效果都必须是圆角/胶囊，不得出现矩形色块。
   ...
   - 2026-06-17：继续收敛圆角按压/选中反馈。Skills 列表、Skill detail 文件行、Skills management hub 行改为 shaped `Surface(onClick)`；消息 action menu 的发送失败行改为圆角 `Surface`；JsonRender fallback URL 文本复用 `clickableJsonUrl`，避免圆角卡片中出现矩形按压块。验证：`:features:skills:impl:compileDebugKotlin`、`:features:messages:impl:compileDebugKotlin`、`:features:messages:impl:testDebugUnitTest --tests JsonSpecRenderTest` 通过。
   ...
   - 2026-06-17：Skills Home 的 Mine/Marketplace segmented tab 从 `background + clickable` 改为 shaped `Surface(onClick)`，选中态、按压态和圆角容器保持同层裁剪。验证：`:features:skills:impl:compileDebugKotlin` 通过。
   - 2026-06-17：timeline 通用 `timelinePressFeedback` 现在把缩放/按压反馈按传入 shape 裁剪；standalone AI/tool/markdown row、普通 bubble、state event container 的长按/按压边界都跟随圆角，不再使用未裁剪矩形层。验证：`:features:messages:impl:compileDebugKotlin` 通过。
...
8. 独立 Vault 管理页（List + Edit，CRUD）已验证；剩余本地化和视觉 polish。
9. Voice Library 功能链路已回归（上传 API、current-recording presenter state、下载缓存式试听、原生 recorder bridge、已录音本地回放/进度/seek/scrub、设计系统 waveform UI、真实 m4a waveform 采样均有测试覆盖）；剩余最终样式、真机录音/拖动手势、设备截图和 iOS 视觉对比。
