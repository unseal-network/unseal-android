...
- Android Credits topup still needs real Stripe PaymentSheet bridge. Settings balance refresh propagation is now wired through `PreferencesFlowNode` -> `PreferencesRootEvent.RefreshCreditBalance` -> `PreferencesRootPresenter.loadCreditBalance()`.
...
| Credits/topup | Credits balance/ledger/usage/analytics, topup | `features/credits` | P1 | Route split exists (`createForUnsealApi` + homeserver). Credits data/state covers balance, ledger pagination, daily usage, analytics period values and partial failure. Topup now has Android state/presenter/child page for preset/custom amount, PaymentIntent creation, success/cancel/failure and status polling. Remaining: native Stripe PaymentSheet bridge, final visual parity and screenshot coverage. |
...
| Credits / Topup | Balance, ledger, usage, analytics, topup, PaymentSheet. | Unseal API for credits/topup, homeserver where iOS uses chatbot client. | Topup state exists; add PaymentSheet external action result model. | Bridge Android Stripe PaymentSheet; match iOS amount selector and success/cancel flow. |
...
| Game Picker | P1 | `RoomScreen/GamePicker/*` | `features/messages/impl/.../messagecomposer/gamepicker` | Android has composer integration, but picker UI/data parity and room menu entry need screenshot verification. |
...
| Feature | Priority | iOS Source | Android Source | Current Gap |
| --- | --- | --- | --- | --- |
| Credits Screen | P1 | `Credits/CreditsScreen/*` | `features/credits/impl/*` | Balance, ledger pagination, daily usage chart, token analytics, low balance, tabs and partial error preservation are implemented in presenter/state tests. Analytics period values now mirror iOS `sevendays/thirtydays/all`. Remaining: visual polish, resource strings, full View test refresh. |
| Topup | P1 | `Credits/TopupScreen/*` | `features/credits/impl/topup/*` | Android now mirrors iOS state semantics: preset/custom amount, `$1-$500` validation, `processing/awaitingPaymentSheet/confirming/success/failed`, PaymentIntent creation, client-secret ID parsing, ledger-settled polling, dismiss completed/cancel callbacks. Remaining: hook `AwaitingPaymentSheet` to Android Stripe PaymentSheet and match iOS screen visuals. |
