...
1. `.github/workflows/release.yml` builds signed Android artifacts and creates the GitHub Release.
2. `.github/workflows/play-release.yml` downloads the AAB from that GitHub Release and uploads it to Google Play.
...
Open GitHub Actions, choose `Create GitHub release`, then run it manually with:
...
The workflow downloads the `*gplay-release.aab` asset from the GitHub Release and uploads it to the selected Google Play track for package `network.unseal`.
